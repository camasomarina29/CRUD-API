...
class Example implements IExample
{
    ...
    // insert user
    public function Insert($data){
        $sql = "INSERT INTO ".$this->table_name."(first_name, last_name, is_admin) VALUES(?, ?, ?)";
        try{
            $stmt = $this->pdo->prepare($sql);

            $isAdmin = filter_var($data->is_admin, FILTER_VALIDATE_BOOLEAN);

            if($stmt->execute([$data->first_name, $data->last_name, $isAdmin])){
     ...
     public function Update($id, $data) {
        $sql = "UPDATE " . $this->table_name . " SET first_name = ?, last_name = ?, is_admin = ? WHERE id = ?";
        try {
            $stmt = $this->pdo->prepare($sql);
            
            $isAdmin = filter_var($data->is_admin, FILTER_VALIDATE_BOOLEAN);
    
            if ($stmt->execute([$data->first_name, $data->last_name, $isAdmin, $id])) {
